...
public class OrganizeDireitosCreditoriosSocopa extends OrganizeDireitosCreditorios
{
	public OrganizeDireitosCreditoriosSocopa(FundoDeInvestimento fundo, int idEntidadeServidor, Login login, Connection conn)	
	{
	...
	public void organizeFile(File file, File folder)
	{
		...
		{
    		File destiny = new File(super.rootDireitosCreditoriosProcessar+separator+file.getName());
    		OperatingSystem.copyRecentFile(file, destiny);
    		try {
				OperatingSystem.delete(file);
			} catch (IOException e) {
