...
public class OrganizeCotasSocopa extends OrganizeCotas
{
	public OrganizeCotasSocopa(FundoDeInvestimento fundo, int idEntidadeServidor, Login login, Connection conn)	
	{
	...
	public void organizeFile(File file, File folder)
	{
		...
		{
			File destiny = new File(super.rootCotasProcessar+separator+file.getName());
    		OperatingSystem.copyRecentFile(file, destiny);
    		try {
				OperatingSystem.delete(file);
			} catch (IOException e) {
