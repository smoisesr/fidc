...
public class HandlerLimiteDeBloqueioSacado 
{

	public HandlerLimiteDeBloqueioSacado() 
	{
	
	}
	
	public static void assessLimiteDeBloqueioSacado(Operacao op, Connection conn)
	{
		ArrayList<LimiteDeConcentracao> limitesDeConcentracao = HandlerLimiteDeBloqueioSacado.readStoredLimiteDeBloqueioSacado(op, conn);
...
//		System.out.println(query);
	...
	public static ArrayList<LimiteDeConcentracao> readStoredLimiteDeBloqueioSacado(Operacao op, Connection conn)
	{
...
//		private String tipoDeConcentracao="";	
		...
		String query = "SELECT * FROM blacklistSacado WHERE idFundo=" + op.getFundo().getIdFundo(); //$NON-NLS-1$
//		System.out.println(query);
		ResultSet rs = null;
		try {
			rs = stmt.executeQuery(query);
