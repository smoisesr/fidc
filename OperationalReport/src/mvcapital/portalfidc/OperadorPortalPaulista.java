...
//	private SshClient sshClient = new SshClient("192.168.2.122", 22, "moises", "preciosa"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	private SshClient sshClient = new SshClient("192.168.1.152", 22, "moises", "preciosa"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$	
	...
	private String rootReportsLocalLinux = ""; //$NON-NLS-1$
	...
	public OperadorPortalPaulista()
	{
		...
		if(this.hostname.toLowerCase().contains("mvcapital12")) //$NON-NLS-1$
		{
			this.clickX = 810;
			this.clickY = 520;
			this.rootDownloadsLinux = "/home/DownloadsPortal2"; //$NON-NLS-1$
		...
		else if(this.hostname.toLowerCase().contains("mvcapital06")) //$NON-NLS-1$
		{
			this.clickX = 670;
			this.clickY = 440;			
			this.rootDownloadsLinux = "/home/DownloadsPortal"; //$NON-NLS-1$
			this.rootDownloadsLocal = "C:\\DownloadsPortal"; //$NON-NLS-1$
			this.rootReportsLocal = "C:\\DownloadsPortal\\Reports"; //$NON-NLS-1$
		...
		this.mysql.connect();	
		OperadorPortalPaulista.conn = (Connection) this.mysql.getConn();
		readManagerAccount();
	}
...
//        		System.out.println("ScrollUp: " + scrollUp);
	    	...
	    	WebElement linkLiquidacao = this.driver.findElement(By.linkText("Liquidação")); //$NON-NLS-1$
...
//    		System.out.println("ScrollUp: " + scrollUp);
    	...
    	List<WebElement> listOfPdf = this.driver.findElements(By.name("pdfLiquidacao")); //$NON-NLS-1$
    	...
    	if(listOfRowsLiquidacao.size()%listOfPdf.size()==0)
    	{
        	...
        	System.out.println("ListOfPdf: " + listOfPdf.size()); //$NON-NLS-1$
        	System.out.println(listOfRowsLiquidacao.size()/listOfPdf.size() + " fields"); //$NON-NLS-1$
        	System.out.println(listOfPdf.size() + " ops"); //$NON-NLS-1$
    	}
...
//    	System.out.println("rowsOperation: " + rowsOperation.size());
    	...
    	for(ArrayList<WebElement> rowOperation:rowsOperation)
    	{
    		...
    		for(Operacao op:this.operations)
    		{
    			if(HandlerOperacoes.equals(operacao, op))
    			{
    				...
    				if(op.isAprovado())
    				{
    					...
    					if(op.getStatuses().get(op.getStatuses().size()-1).getDescription().contains("aprovação do Gestor")) //$NON-NLS-1$
    					//if(op.getStatus().get(op.getStatus().size()-1).getDescription().contains("Reprovado"))
    		    		{
    						...
    						{
    							System.out.println("Não é gestor de TED"); //$NON-NLS-1$
    		        			if(!op.isHaveTransferDetailsChecked())
    		        			{
    		        				readOperationTransferDetails(rowOperation, op);
    				    			HandlerOperacoes.updateStore(op, conn);
    		...
    		if(!existOp)
    		{
    			...
    			{
    				System.out.println("Aprovado");    				 //$NON-NLS-1$
    				if(operacao.getStatuses().get(operacao.getStatuses().size()-1).getDescription().contains("aprovação do Gestor")) //$NON-NLS-1$
...
//	    System.out.println("Perl command executed!");
	    ...
	    String command="perl " + this.getRootReportsLocalLinux() + "/generateOperationsSummaryWeb.pl"; //$NON-NLS-1$ //$NON-NLS-2$
	    System.out.println("command: " + command); //$NON-NLS-1$
	    this.sshClient.executeCommandOutput(command);		 
	...
	{    	
    	if(operacao.isAprovado())
    	{
    		System.out.println("\nOperação " + operacao.getFundo().getNome() + " " + operacao.getCedente().getNome() +  " "+ operacao.getNomeArquivo() + " aprovada!"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    	}	    	
...
//    	System.out.println("\nMore Data");
		...
		{	
	    	...
	    	{
	    		System.out.println("Nome do fundo não coincide!"); //$NON-NLS-1$
	    	...
	    	{
	    		System.out.println("Nome do Arquivo não coincide!"); //$NON-NLS-1$
	    		System.out.println("Tentando novamente!"); //$NON-NLS-1$
	    		//this.approuve();	//Recursive attempt    		
	    	}
	    	else
	...
	//    		System.out.println("nomeArquivo: " + nomeArquivo);
	    	...
	    	{
		    	System.out.println("\nOperação Conta Interna Paulista");	    	 //$NON-NLS-1$
	    	...
	    	System.out.println("\nOperação Conta Externa Paulista"); //$NON-NLS-1$
		...
		{
			System.out.println("--Não foi possivel ler o nome do fundo nem os outros detalhes--"); //$NON-NLS-1$
		}
    	this.setJustReadedTransferDetails(true);
	}
...
//        System.out.println("ValorDeTransferencia: " + weValorDeTransferencia.getText());
    	...
    	System.out.println("Fundo: " + currentOperation.getFundo().getNomeCurto()); //$NON-NLS-1$
...
//								System.out.println(lineClean);
	...
	public static void readManagerAccount()
	{
		try 
		{
			System.out.println("MVCapital - Usuário de Portal"); //$NON-NLS-1$
...
//		driver = new ChromeDriver();
        ...
        this.calLimit.set(Calendar.HOUR_OF_DAY, 18);
        this.calLimit.set(Calendar.MINUTE, 50);
